...
const GROUP_CONTROLLER_DAYS_OF_WEEK = [
	'monday',
	'tuesday',
	'wednesday',
	'thursday',
...
const getAttendanceDayOfWeekName = date => {
	const localDate = new Date(date.getTime() + 5 * 60 * 60 * 1000)
	return GROUP_CONTROLLER_DAYS_OF_WEEK[(localDate.getUTCDay() + 6) % GROUP_CONTROLLER_DAYS_OF_WEEK.length]
}

const getCurrentLessonSchedule = date => {
	const localDate = new Date(date.getTime() + 5 * 60 * 60 * 1000)
	const startMinutes = Math.max(0, localDate.getUTCHours() * 60 + localDate.getUTCMinutes() - 5)
	const hours = String(Math.floor(startMinutes / 60)).padStart(2, '0')
	const minutes = String(startMinutes % 60).padStart(2, '0')
	return {
		dayOfWeek: getAttendanceDayOfWeekName(date),
...
const runTests = async () => {
	...
	try {
		...
		await test('permission middleware enforces all required permissions', async () => {
			await withPatchedMethods(
				[
					...
					[Student, 'findById', async () => ({ _id: studentId, originalCoursePrice: 0, activeDiscountAmount: 0 })],
				],
				async () => {
		...
		await test('admin bypasses attendance window and date checks', async () => {
			const groupId = '507f1f77bcf86cd799439121'
			const teacherId = '507f1f77bcf86cd799439122'
			const studentId = '507f1f77bcf86cd799439123'
				...
				[
					[
						...
						() => makeQuery({
							...
							teacher: teacherId,
							supportTeachers: [],
							schedule: [getCurrentLessonSchedule(new Date())],
							attendance: [],
							save: async () => {},
						}),
					],
					[Student, 'find', async () => [{ _id: studentId, monthlyExtraLessonRequestsUsed: 0, monthlyExtraLessonsUsed: 0 }]],
					[Student, 'aggregate', async () => []],
				],
				async () => {
					const res = await callHandler(groupController.upsertGroupAttendance, {
						params: { groupId },
						user: { _id: teacherId, role: 'admin' },
						body: {
		...
		await test('teacher cannot edit after 24h or after one patch', async () => {
			const groupId = '507f1f77bcf86cd799439131'
			const teacherId = '507f1f77bcf86cd799439132'
			const studentId = '507f1f77bcf86cd799439133'
			const now = new Date()

			// schedule was 26 hours ago
			const pastDate = new Date(now.getTime() - 26 * 60 * 60 * 1000)
			...
			await withPatchedMethods(
				[
					[
						...
						async () => ({
							...
							teacher: teacherId,
							supportTeachers: [],
							schedule: [{ dayOfWeek: 'monday', startTime: '09:00', durationMinutes: 90 }],
							attendance: [
								{ student: studentId, date: pastDate, status: 'absent', updatedOnceByTeacher: false },
							],
						}),
					],
				],
				async () => {
					const res = await callHandler(groupController.markGroupAttendanceStudent, {
						params: { groupId, studentId },
						user: { _id: teacherId, role: 'teacher' },
						body: { date: pastDate.toISOString(), status: 'present' },
					})
			...
			// now simulate existing updatedOnceByTeacher true
				...
				[
					[
						...
						async () => ({
							...
							teacher: teacherId,
							supportTeachers: [],
							schedule: [getCurrentLessonSchedule(currentDate)],
							attendance: [
								{ student: studentId, date: currentDate, status: 'absent', updatedOnceByTeacher: true },
							],
							save: async () => {},
						}),
					],
					[Student, 'exists', async () => true],
					[Student, 'findById', async () => ({ _id: studentId, originalCoursePrice: 0, activeDiscountAmount: 0 })],
					[Student, 'findByIdAndUpdate', async () => ({ _id: studentId })],
				],
				async () => {
					const res2 = await callHandler(groupController.markGroupAttendanceStudent, {
						params: { groupId, studentId },
						user: { _id: teacherId, role: 'teacher' },
						body: { date: currentDate.toISOString(), status: 'present' },
					})

					assert.strictEqual(res2.statusCode, 403)
					assert.strictEqual(res2.body.code, 'ATTENDANCE_PATCH_LIMIT')
				},
			)
		})

		await test('computeStudentPayableAmount respects discount', async () => {
			const finance = require('../src/services/finance.service')
			const student = { originalCoursePrice: 800000, activeDiscountAmount: 400000 }
			const payable = finance.computeStudentPayableAmount(student)
			assert.strictEqual(payable, 400000)
		})
		...
		await test('extra-lesson completion charges paid students and credits teacher', async () => {
			...
			await withPatchedMethods(
				[
					[ExtraLesson, 'findById', () => makeQuery({
						_id: lessonId,
						assignedTeacher: teacherId,
						students: [{ student: studentFree._id }, { student: studentPaid._id }],
						status: 'confirmed',
					})],
					[ExtraLesson, 'findOneAndUpdate', async () => ({
						_id: lessonId,
						assignedTeacher: teacherId,
						students: [{ student: studentFree._id }, { student: studentPaid._id }],
						status: 'completed',
					...
					[Student, 'findById', async id => {
						if (String(id) === String(studentFree._id)) return Object.assign(studentFree, { save: async () => {} })
						if (String(id) === String(studentPaid._id)) return Object.assign(studentPaid, { save: async () => {} })
						return null
					}],
					[require('../src/services/finance.service'), 'processExtraLessonPayment', async ({ studentId, teacherId, amount }) => {
						processed.push({ studentId: String(studentId), teacherId: String(teacherId), amount })
						return true
					}],
				],
				async () => {
					const res = await callHandler(require('../src/controllers/extra-lesson.controller').markCompleted, {
						params: { lessonId },
						user: { _id: 'some-manager', role: 'admin' },
						body: {},
					})
					assert.strictEqual(res.statusCode, 200)
