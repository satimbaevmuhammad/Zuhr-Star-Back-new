...
| Роли | teacher, supportTeacher, headteacher, admin, superadmin | Нет ролей |
...
superadmin   — полный доступ ко всему
  └── admin       — управление сотрудниками, финансами, группами
        └── headteacher  — руководит учителями, может добавлять бонусы/штрафы
              └── supportTeacher  — поддержка, проводит доп.уроки
...
| `role` | string | ✅ | teacher, supportTeacher, headteacher, admin |
...
> **Внимание:** `supportTeachers` в группе — это НЕ то же самое, что `isExtraLessonSupport` у пользователя. Первое — помощники конкретной группы, второе — глобальные учителя доп.уроков.
...
Authorization: Bearer <employee-token>   (supportTeacher, headteacher, admin, superadmin)
Content-Type: application/json
...
{
  "_id": "65f...",
  "assignedTeacher": { "_id": "65f...", "fullname": "...", "phone": "...", "role": "supportTeacher" },
  "scheduledAt": "2026-03-28T10:10:00.000Z",
  "subject": "Алгебра",
  "requestType": "student_request",
...
- [ ] Роль `supportTeacher` ≠ `isExtraLessonSupport` — это разные вещи!
  - `supportTeacher` — роль сотрудника в иерархии компании
