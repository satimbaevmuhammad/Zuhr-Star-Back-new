...
const EXTRA_LESSON_PRICE = 15000

const computeTeacherSlots = workSchedule => {
	const startTime = (workSchedule && workSchedule.startTime) || '14:00'
	const endTime = (workSchedule && workSchedule.endTime) || '20:00'
	...
	const slots = []
	let cur = startMins
	while (cur + LESSON_DURATION_MINUTES <= endMins) {
...
const isAllowedUtcSlotDate = (scheduledAt, slotSet = SLOT_SET) => {
...
const resolveScheduleFromDateAndSlot = (dateValue, slotValue, slotSet = SLOT_SET) => {
...
const resolveScheduleFromBody = ({ date, slot, scheduledAt }, slotSet = SLOT_SET) => {
	...
	if (hasDateOrSlot) {
		...
		return resolveScheduleFromDateAndSlot(date, slot, slotSet)
	}

	const parsed = parseScheduledAtInput(scheduledAt)
	if (!parsed || !isAllowedUtcSlotDate(parsed, slotSet)) {
...
const assertTeacherDailyLimit = async ({ teacherId, localDate, session, excludeLessonId = null, maxLessons = MAX_LESSONS_PER_DAY }) => {
...
const assertSupportTeacher = async ({ teacherId, session }) => {
	const teacherQuery = User.findById(teacherId).select('fullname role isExtraLessonSupport workSchedule')
	...
	if (teacher.isExtraLessonSupport !== true) {
		throw createHttpError(400, 'Teacher is not designated as extra lesson support', 'BAD_REQUEST', 'teacherId')
	}
...
exports.getAvailability = async (req, res) => {
	try {
		...
		const teacher = await User.findById(teacherId).select('fullname isExtraLessonSupport workSchedule')
		if (!teacher) {
			return sendError(res, 404, 'Teacher not found', 'NOT_FOUND', 'teacherId')
		}
		if (teacher.isExtraLessonSupport !== true) {
			return sendError(res, 400, 'Teacher is not designated as extra lesson support', 'BAD_REQUEST', 'teacherId')
		}
		...
		const teacherSlots = computeTeacherSlots(teacher.workSchedule)
		const slots = teacherSlots.map(slot => {
...
exports.bookLesson = async (req, res) => {
	try {
		...
		const teacher = await User.findById(teacherId).select('isExtraLessonSupport workSchedule')
		if (!teacher) {
			return sendError(res, 404, 'Teacher not found', 'NOT_FOUND', 'teacherId')
		}
		if (teacher.isExtraLessonSupport !== true) {
		...
		const teacherSlots = computeTeacherSlots(teacher.workSchedule)
		const teacherSlotSet = new Set(teacherSlots)
		...
		let schedule
		try {
			schedule = resolveScheduleFromBody(req.body, teacherSlotSet)
		} catch (error) {
			if (isHttpError(error)) {
				return sendError(res, error.statusCode, error.message, error.code, error.field)
			}
		...
		const result = await runWithOptionalTransaction(async session => {
			...
			await assertTeacherDailyLimit({
				...
				session,
				maxLessons: teacherSlots.length,
			})
		...
		// increment student's monthly request counter (reset month if needed)
		try {
			const studentId = String(req.student._id)
			...
			if (student) {
				...
				if (student.extraLessonQuotaMonth !== month) {
					student.extraLessonQuotaMonth = month
					student.monthlyExtraLessonRequestsUsed = 0
					student.monthlyExtraLessonsUsed = 0
				}
				student.monthlyExtraLessonRequestsUsed = (student.monthlyExtraLessonRequestsUsed || 0) + 1
				await student.save()
			}
		} catch (err) {
			console.error('Failed to update student extra-lesson request quota:', err)
		}
...
exports.markCompleted = async (req, res) => {
	try {
		...
		// Process payments for students who exceeded free quota
		try {
			const service = require('../services/finance.service')
			const now = new Date()
			const month = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`

			// Process each student entry sequentially (transactions handled in service)
			for (const entry of (updated.students || [])) {
				try {
					...
					// If student has free completed lessons remaining, consume one
					if ((student.monthlyExtraLessonsUsed || 0) < 3) {
						student.monthlyExtraLessonsUsed = (student.monthlyExtraLessonsUsed || 0) + 1
						await student.save()
						continue
					}

					// Otherwise, process paid extra lesson: deduct from student, credit teacher
					await service.processExtraLessonPayment({
						studentId: student._id,
						teacherId: updated.assignedTeacher,
						amount: EXTRA_LESSON_PRICE,
						actorId: req.user?._id,
					})
				} catch (err) {
					console.error('Failed processing extra lesson payment for student', entry.student, err)
				}
			}
		} catch (err) {
			console.error('Error processing extra-lesson payments:', err)
		}
...
exports.createExtraLesson = async (req, res) => {
	try {
		...
		if (!isManager && req.userDocument?.isExtraLessonSupport !== true) {
			return sendError(
				res,
				403,
				'Only designated extra lesson support teachers or admin can create extra lessons directly',
				'FORBIDDEN',
			)
		}

		if (isManager && !explicitTeacherId && req.userDocument?.isExtraLessonSupport !== true) {
		...
		const assignedTeacher = await assertSupportTeacher({ teacherId: assignedTeacherId })
		const assignedTeacherSlots = computeTeacherSlots(assignedTeacher.workSchedule)
		const assignedTeacherSlotSet = new Set(assignedTeacherSlots)

		let schedule
		try {
			schedule = resolveScheduleFromBody(req.body, assignedTeacherSlotSet)
		} catch (error) {
			if (isHttpError(error)) {
				return sendError(res, error.statusCode, error.message, error.code, error.field)
			}
		...
		const lesson = await runWithOptionalTransaction(async session => {
			...
			await assertTeacherDailyLimit({
				...
				localDate: schedule.localDate,
				session,
				maxLessons: assignedTeacherSlots.length,
			})
...
exports.updateExtraLesson = async (req, res) => {
	try {
		...
		if (hasSchedulingChange) {
			...
			const lessonTeacher = await User.findById(lesson.assignedTeacher).select('workSchedule')
			const lessonTeacherSlots = computeTeacherSlots(lessonTeacher?.workSchedule)
			const lessonTeacherSlotSet = new Set(lessonTeacherSlots)

			let schedule
			try {
				schedule = resolveScheduleFromBody(req.body, lessonTeacherSlotSet)
			} catch (error) {
				if (isHttpError(error)) {
					return sendError(res, error.statusCode, error.message, error.code, error.field)
				}
			...
			await assertTeacherDailyLimit({
				...
				maxLessons: lessonTeacherSlots.length,
			})
...
exports.updateMySchedule = async (req, res) => {
	try {
		const userId = String(req.user?._id || req.user?.id || '')
		const user = await User.findById(userId).select('isExtraLessonSupport workSchedule')
		if (!user) {
			return sendError(res, 404, 'User not found', 'NOT_FOUND')
		}
		if (user.isExtraLessonSupport !== true) {
			return sendError(res, 403, 'Only designated extra lesson support teachers can update their work schedule', 'FORBIDDEN')
		}

		const rawStart = req.body.startTime
		const rawEnd = req.body.endTime

		if (typeof rawStart === 'undefined' && typeof rawEnd === 'undefined') {
			return sendError(res, 400, 'startTime or endTime is required', 'BAD_REQUEST')
		}

		const currentStart = (user.workSchedule && user.workSchedule.startTime) || '14:00'
		const currentEnd = (user.workSchedule && user.workSchedule.endTime) || '20:00'

		const startTime = rawStart ? normalizeSlotInput(String(rawStart).trim()) : currentStart
		const endTime = rawEnd ? normalizeSlotInput(String(rawEnd).trim()) : currentEnd

		if (rawStart && !startTime) {
			return sendError(res, 400, 'startTime must be in HH:mm format', 'BAD_REQUEST', 'startTime')
		}
		if (rawEnd && !endTime) {
			return sendError(res, 400, 'endTime must be in HH:mm format', 'BAD_REQUEST', 'endTime')
		}
		...
		if (endMins - startMins < LESSON_DURATION_MINUTES) {
			return sendError(
				res,
				400,
				`Schedule must allow at least one ${LESSON_DURATION_MINUTES}-minute slot`,
				'BAD_REQUEST',
				'endTime',
			)
		}

		user.workSchedule = { startTime, endTime }
		await user.save()

		const slots = computeTeacherSlots(user.workSchedule)
		return res.status(200).json({
			message: 'Work schedule updated',
			workSchedule: user.workSchedule,
			slots,
			maxLessonsPerDay: slots.length,
		})
	} catch (error) {
		return handleControllerError(res, error, 'updateMySchedule')
	}
}
