...
// Achievements: add a polymorphic achievement (certificate, badge, other)
exports.addAchievement = async (req, res) => {
	try {
		...
		// permission: require admin or owner
		...
		const payload = {
			...
			awardedAt: req.body.awardedAt ? new Date(req.body.awardedAt) : undefined,
			issuedAt: req.body.issuedAt ? new Date(req.body.issuedAt) : undefined,
			expiresAt: req.body.expiresAt ? new Date(req.body.expiresAt) : null,
			fileUrl: req.file ? `/uploads/${req.file.filename}` : (req.body.fileUrl || null),
			credentialId: req.body.credentialId || null,
			meta: req.body.meta ? JSON.parse(String(req.body.meta)) : {},
			awardedBy: req.user.id,
		}

		user.achievements.push(payload)
		await user.save()

		const added = user.achievements[user.achievements.length - 1]
...
exports.listAchievements = async (req, res) => {
	try {
		...
		// Owner or admins can list
		const isOwner = req.user && String(req.user.id) === String(userId)
		const isAdmin = req.user && ['admin', 'superadmin'].includes(req.user.role)
		if (!isOwner && !isAdmin) return res.status(403).json({ message: 'Forbidden' })

		return res.status(200).json(user.achievements || [])
	} catch (error) {
		console.error('List achievements failed:', error)
		return res.status(500).json({ message: 'Internal server error' })
	}
}
...
exports.revokeAchievement = async (req, res) => {
	try {
		...
		if (!userId || !userId.match(/^[0-9a-fA-F]{24}$/) || !achId || !achId.match(/^[0-9a-fA-F]{24}$/)) {
		...
		const idx = user.achievements.findIndex(a => String(a._id) === String(achId))
		if (idx === -1) return res.status(404).json({ message: 'Achievement not found' })

		user.achievements[idx].revoked = true
		user.achievements[idx].revokedAt = new Date()
		await user.save()

		return res.status(204).send()
	} catch (error) {
		console.error('Revoke achievement failed:', error)
		return res.status(500).json({ message: 'Internal server error' })
	}
}
