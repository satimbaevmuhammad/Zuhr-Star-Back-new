...
// Admin endpoint to set/update student discount
exports.setStudentDiscount = async (req, res) => {
	try {
		...
		if (!Number.isFinite(originalCoursePrice) || originalCoursePrice < 0) return res.status(400).json({ message: 'originalCoursePrice must be non-negative number' })
		if (!Number.isFinite(activeDiscountAmount) || activeDiscountAmount < 0) return res.status(400).json({ message: 'activeDiscountAmount must be non-negative number' })

		// update student payable
		...
		return res.status(200).json({ message: 'Student discount updated', event, student })
	} catch (error) {
		console.error('Set student discount failed:', error)
		return res.status(500).json({ message: 'Internal server error' })
	}
}
