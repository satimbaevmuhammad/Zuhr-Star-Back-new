...
const { requireAuth, requireStudentAuth, requireAnyAuth, allowPermissions } = require('../middleware/auth.middleware')
 ...
 */
router.get('/availability', requireAnyAuth, extraLessonController.getAvailability)
 ...
 *       The caller must be a support teacher (role: 'supporteacher'), or an admin who provides
...
// ─── SUPPORT TEACHER: OWN SCHEDULE ───────────────────────────────────────────

/**
 * @swagger
 * /api/extra-lessons/my-schedule:
 *   patch:
 *     tags: [ExtraLessons]
 *     summary: Update the authenticated support teacher's schedule
 *     description: |
 *       Allows a support teacher to update their own extra lesson schedule.
 *       The caller must be authenticated and must be an extra lesson support teacher.
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
 ...
 *                   example: 6
 *       400:
 *         description: Validation error or invalid schedule data
 *       401:
 *         description: Authentication required
 *       403:
 *         description: Only support teachers can update schedules
 *       404:
...
router.patch(
  '/my-schedule',
  requireAuth,
  extraLessonController.updateMySchedule
)
